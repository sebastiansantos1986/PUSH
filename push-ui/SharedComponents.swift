...
struct TrafficLights: View {
    ...
    private var isForced: Bool {
        // During forced install ALL states are locked — user cannot close any popup
        ctx.forcedInstall
            || ctx.state == .downloading
            || ctx.state == .installing
            || ctx.state == .hardBlock
            || ctx.state == .passwordPrompt
            || ctx.state == .preflightDisk
            || ctx.state == .preflightPower
            || ctx.state == .rebooting
    }
    ...
    var body: some View {
        HStack(spacing: 8) {
            if !isForced {
                // Only show close button when NOT in a forced install
                trafficButton(color: Color(red: 1.0, green: 0.37, blue: 0.34),
                              hoverIcon: "xmark",
                              action: { Foundation.exit(PushExitCode.dismiss.rawValue) })
            } else {
                // Greyed out — visually present but does nothing
                Circle()
                    .fill(Color.gray.opacity(0.3))
                    .frame(width: 12, height: 12)
            }
            // Minimize always disabled during forced install
            if !isForced {
...
// MARK: - Date picker sheet (Schedule…)
//
// Lets the user pick a specific future date and time to be reminded about the
// update. Quick-pick chips for common cases (1 day, 3 days, until deadline)
// plus a full DatePicker for arbitrary times. Always capped to the deadline —
// the picker won't allow a time past the enforcement deadline.

struct DatePickerSheet: View {
    let config:    UIConfig
    let deadline:  Date            // user can't pick a time past this
    let onConfirm: (Date) -> Void
    let onCancel:  () -> Void

    // Default: 1 day from now, clamped to deadline
    ...
    // Three quick-pick options. Each is clamped so we never offer a time past deadline.
    private var quickPicks: [(label: String, date: Date)] {
        let now = Date()
        let in1Day  = now.addingTimeInterval(86400)
        let in3Days = now.addingTimeInterval(3 * 86400)
        var picks: [(String, Date)] = []
        if in1Day  < deadline { picks.append(("Tomorrow",     in1Day)) }
        if in3Days < deadline { picks.append(("In 3 days",    in3Days)) }
        // Always offer "until deadline" as the last resort
        picks.append(("Until deadline", deadline.addingTimeInterval(-3600)))
        return picks
    }
    ...
    private var chosenFormatted: String {
        let f = DateFormatter()
        f.dateStyle = .full
        f.timeStyle = .short
        return f.string(from: chosen)
    }
    ...
    var body: some View {
        VStack(spacing: 18) {
            VStack(spacing: 6) {
                ...
                Text("Choose when you'd like to be reminded about this update.")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
            ...
            // Quick-pick chips
            HStack(spacing: 8) {
                ForEach(0..<quickPicks.count, id: \.self) { i in
                    Button {
                        chosen = quickPicks[i].date
                    } label: {
                        Text(quickPicks[i].label)
                            .font(.system(size: 12, weight: .medium))
                    ...
                    .background(
                        Capsule().fill(
                            isSameMinute(chosen, quickPicks[i].date)
                                ? accent.opacity(0.15)
            ...
            VStack(spacing: 4) {
                Text("You'll be reminded on")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
                Text(chosenFormatted)
                    .font(.system(size: 13, weight: .medium))
                    .foregroundStyle(accent)
            }
            .padding(.top, 4)

            Text("Deadline is \(deadlineFormatted). After that, the update will install automatically.")
                .font(.system(size: 10))
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            ...
            HStack(spacing: 10) {
                Button("Cancel", action: onCancel)
                    .buttonStyle(PushSecondaryButtonStyle())
                Spacer()
                Button("Schedule") { onConfirm(chosen) }
